              ...
              <tr *ngFor="let apt of pendingApts">
                ...
                <td style="text-align:right">
                  <button mat-raised-button color="primary"
                    ...
                    <mat-icon style="font-size:14px;margin-right:4px">receipt_long</mat-icon>
                    Facturer
                  </button>
                </td>
              </tr>
            ...
            <tbody>
              <tr *ngFor="let inv of invoices">
                ...
                <td style="text-align:right">
                  ...
                  <button mat-icon-button (click)="downloadFeuilleSoins(inv)" title="Feuille de soins">
...
export class SecretaryInvoicesComponent implements OnInit {
  ...
  tab: 'pending' | 'all' | 'overdue' = 'pending';
  ...
  private loadPending(): void {
    ...
    // Load both invoices and completed appointments, then cross-reference
  ...
  facturer(apt: Appointment): void {
    ...
    ref.afterClosed().subscribe((result: FacturerDialogResult | undefined) => {
      ...
      this.invoiceSvc.create({
        ...
        error: err => {
          this.snack.open(err.error?.message || 'Erreur lors de la création.', 'OK', { duration: 4000 });
        }
      });
    });
  }
  ...
  download(inv: Invoice): void {
    this.invoiceSvc.downloadPdf(inv._id).subscribe(blob => {
      ...
      a.download = `Facture-${inv._id}.pdf`; a.click();
  ...
  downloadFeuilleSoins(inv: Invoice): void {
    this.invoiceSvc.downloadFeuilleSoins(inv._id).subscribe(blob => {
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href = url;
      a.download = `Feuille-soins-${inv._id}.pdf`; a.click();
      URL.revokeObjectURL(url);
    });
  }
